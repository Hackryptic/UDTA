...
if __name__ == "__main__":
    ...
    aircraft_dataset = Aircraft("/mnt/ssd/aircraft", train="trainval", transform=preprocess_augment, download=False)
    ...
    #test_dataset = pytorch_dataset.cub2011.Cub2011("/mnt/ssd/CUB", train=False, transform=preprocess_augment, download=False)

    #aircraft_dataset = pytorch_dataset.cars.Cars("/mnt/ssd/stan_cars", train=True, transform=preprocess_augment, download=False)
    #test_dataset = pytorch_dataset.cars.Cars("/mnt/ssd/stan_cars", train=False, transform=preprocess_augment, download=False)
    ...
    model = models.mobilenetv2_org_thin_sf.mobilenet_v2_org_thin_sf(pretrained=False)
    #model = models.mobilenetv2_org_thin_ne.mobilenet_v2_org_thin_ne(pretrained=False)
    ...
    #finetune_keyword_list =["thin", "pre_layers.0.1", "conv.0.1", "post.0", "classifier"]
    #finetune_keyword_list =["classifier", "inverted_residuals", "thin"]
    finetune_keyword_list =["thin", "classifier"]
    ...
    #print(num_classes)
    #num_classes = 196
    #num_classes = aircraft_dataset.num_classes
    ...
    model_save_path = "./saved_models/thin_org_testo.pth"
    ...
    ae_epoch = 0
    ae_decay_start_epoch = 80

    # Thin Adapter Training Epoch
    thin_epoch = 120
    thin_decay_start_epoch = 60
    ...
    for epoch in range(ae_epoch):
        ...
        for inputs, labels in loader:
            ...
            temp_loss = temp_loss / len(output_list)
    ...
    for epoch in range(thin_epoch):
        ...
        load_time_start = time.time()
        train_time_epoch = time.time() - time.time()
        print("train time epoch: {}".format(train_time_epoch))
        ...
        for inputs, labels in loader:
            ...
            end_time_step = start_time_step - time.time()

            train_time_epoch += end_time_step
