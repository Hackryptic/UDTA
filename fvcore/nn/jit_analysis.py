...
class JitModelAnalysis:
    ...
    def total(self, module_name: str = "") -> int:
        ...
        """
        back_stats, stats = self._analyze()
        module_name = self.canonical_module_name(module_name)
        total_count = sum(stats.counts[module_name].values())
        back_count = sum(back_stats.counts[module_name].values())
        return back_count, total_count
    ...
    def _analyze(self) -> "Statistics":
        ...
        for _, mod in _named_modules_with_dup(self._model):
            ...
            #print(name)
            counts[name] = Counter()
            back_counts[name] = Counter()
            unsupported_ops[name] = Counter()

        all_seen = set()

        #print(self._op_handles)
        ...
        for node in graph.nodes():
            ...
            if kind not in self._op_handles:
                ...
                #print("inputs: {}".format(inputs[0].requires_grad()))
                ...
                for name in ancestors:
                    ...
                    #back_counts[name] += op_counts * 2

                if outputs[0].requires_grad() == True:
                    for name in ancestors:
                        #print("lovvve tyger")
                        #print(name)
                        back_counts[name] += (op_counts + op_counts)
                        #back_counts[name] += op_counts * 2
                #else:
        ...
        back_stats = Statistics(
            counts=back_counts, unsupported_ops=unsupported_ops, uncalled_mods=uncalled_mods
        )

        self._stats = stats
        self._warn_unsupported_ops(unsupported_ops[""])
        self._warn_uncalled_mods(uncalled_mods)
        return back_stats, stats
