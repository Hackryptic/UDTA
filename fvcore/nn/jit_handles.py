...
def mm_flop_jit(inputs: List[Any], outputs: List[Any]) -> Number:
    """
    Count flops for fully connected layers.
    """
    # Count flop for nn.Linear
...
def div_flop_jit(inputs: List[Any], outputs: List[Any]) -> Number:
    """
    Count flops for fully connected layers.
    """
    # Count flop for nn.Linear
...
def add_flop_jit(inputs: List[Any], outputs: List[Any]) -> Number:
    """
    Count flops for Addition.
    """
    # Count flop for add
    # inputs is a list of length 3.
    input_shapes = [get_shape(v) for v in inputs[1:4]]
    # input_shapes[0]: [batch size, input feature dimension]
    # input_shapes[1]: [batch size, output feature dimension]
    #print(input_shapes)
    assert len(input_shapes[0]) == 4, input_shapes[0]
    #assert len(input_shapes[1]) == 4, input_shapes[1]
    batch_size, channel, width, height = input_shapes[0]
    #output_dim = input_shapes[1][1]
    flops = batch_size * (channel * width * height)
    return flops
