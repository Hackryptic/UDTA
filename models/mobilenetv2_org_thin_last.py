...
class ConvBNReLUAE(nn.Module):
    def __init__(self, inp, oup, kernel_size=3, stride=1, groups=1, norm_layer=None, out_point_dim=-1):
        super(ConvBNReLUAE, self).__init__()
        padding = (kernel_size - 1) // 2
...
class ThinBlock(nn.Module):
    def __init__(self, inp, oup, stride, norm_layer=None, use_residual=True, pw_input=-1):
        ...
        layers.extend([
            #ConvBNReLU(inp, oup, stride=stride, kernel_size=1, groups=1, norm_layer=norm_layer),
            ConvBNReLU(inp, inp, stride=stride, kernel_size=3, groups=inp, norm_layer=norm_layer),
...
class MobileNetV2(nn.Module):
    def __init__(self,
        ...
        thin_adapter_setting = [
            ...
            #[120, 160, 1],
            [320, 512, 1],
            [512, self.thin_output_channel, 1]
        ]
        ...
        thin_adapter_setting = [
            ...
            [96, 120, 2],
            [120, 160, 1],
            [160, self.thin_output_channel, 1]
        ]
        '''
        ...
        features.append(cbrae(input_channel, self.last_channel, kernel_size=1, norm_layer=norm_layer, out_point_dim=512))
